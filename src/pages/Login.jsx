...
function RegisterModal({ onClose }) {
  const [form, setForm] = useState({ nombre_completo: '', correo_electronico: '', telefono: '', password: '', confirm: '' });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setErrors({ general: err.error || 'Error al registrar. Intenta de nuevo.' });
    }
    setLoading(false);
  };
    ...
    return (
      <div style={{
        ...
        <div className="mg-card" style={{ width: '100%', maxWidth: 380, padding: 32, textAlign: 'center' }}>
          ...
          <h3 style={{ fontFamily: 'Bebas Neue', fontSize: 24, color: 'var(--text-primary)', marginBottom: 8, letterSpacing: '0.1em' }}>
            CUENTA CREADA
          </h3>
          ...
          <button onClick={onClose} style={{
            ...
            fontFamily: 'JetBrains Mono', fontSize: 11, letterSpacing: '0.12em', color: '#fff',
          }}>
            ENTENDIDO
          </button>
        </div>
      ...
      <div className="mg-card" style={{ width: '100%', maxWidth: 420, padding: 28, maxHeight: '90vh', overflowY: 'auto' }}>
        ...
        {errors.general && (
          <div style={{ padding: '10px 12px', background: 'var(--accent-soft)', border: '1px solid var(--accent-border)', borderRadius: 8, marginBottom: 14 }}>
            <span style={{ fontFamily: 'JetBrains Mono', fontSize: 10, color: 'var(--accent)' }}>{errors.general}</span>
          </div>
        )}

        <Field label="NOMBRE COMPLETO"     value={form.nombre_completo}    onChange={f('nombre_completo')}    error={errors.nombre_completo}    required />
        {errors.nombre_completo && <p style={{ fontFamily: 'JetBrains Mono', fontSize: 9, color: 'var(--accent)', marginTop: -10, marginBottom: 10 }}>{errors.nombre_completo}</p>}

        <Field label="CORREO ELECTRÓNICO"  value={form.correo_electronico} onChange={f('correo_electronico')} type="email" required />
        {errors.correo_electronico && <p style={{ fontFamily: 'JetBrains Mono', fontSize: 9, color: 'var(--accent)', marginTop: -10, marginBottom: 10 }}>{errors.correo_electronico}</p>}

        <Field label="TELÉFONO (opcional)"  value={form.telefono}           onChange={f('telefono')}           placeholder="+51 9XX XXX XXX" />

        <Field label="CONTRASEÑA"          value={form.password}           onChange={f('password')}           type="password" required />
        {errors.password && <p style={{ fontFamily: 'JetBrains Mono', fontSize: 9, color: 'var(--accent)', marginTop: -10, marginBottom: 10 }}>{errors.password}</p>}

        <Field label="CONFIRMAR CONTRASEÑA" value={form.confirm}           onChange={f('confirm')}            type="password" required />
        {errors.confirm && <p style={{ fontFamily: 'JetBrains Mono', fontSize: 9, color: 'var(--accent)', marginTop: -10, marginBottom: 10 }}>{errors.confirm}</p>}

        <p style={{ fontFamily: 'JetBrains Mono', fontSize: 8, color: 'var(--text-faint)', marginBottom: 16, lineHeight: 1.6 }}>
          Tu cuenta sera revisada y activada por el administrador antes de que puedas acceder al sistema.
        </p>
        ...
        <button onClick={handleSubmit} disabled={loading} style={{
          ...
          {loading
            ? <><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"
                ...
                <path d="M21 12a9 9 0 1 1-6.219-8.56"/></svg>REGISTRANDO...</>
            : 'CREAR CUENTA'
          }
        </button>
      </div>
    ...
    <div style={{
      ...
      <div style={{ position: 'relative', zIndex: 1, width: '100%', maxWidth: 360, padding: '0 24px' }} className="anim-fade">
        ...
        <div className="mg-card" style={{ padding: 24 }}>
          <Field label="CORREO ELECTRÓNICO" type="email" value={correo} onChange={setCorreo} placeholder="tu@correo.com" required />
          ...
          <button onClick={handleLogin} disabled={loading || !correo || !password}
            onKeyDown={e => e.key === 'Enter' && handleLogin()}
            style={{
              width: '100%', padding: 13, borderRadius: 10,
              background: (!correo || !password || loading) ? 'var(--bg-surface)' : 'var(--accent)',
              border: `1px solid ${(!correo || !password || loading) ? 'var(--border)' : '#ff5040'}`,
              cursor: (!correo || !password || loading) ? 'not-allowed' : 'pointer',
              fontFamily: 'JetBrains Mono', fontSize: 12, letterSpacing: '0.15em',
              color: (!correo || !password || loading) ? 'var(--text-muted)' : '#fff',
              display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8,
              transition: 'all .25s', marginBottom: 12,
              boxShadow: correo && password && !loading ? '0 0 20px var(--accent-glow)' : 'none',
            }}>
            {loading
              ? <><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"
                  style={{ animation: 'spin-cw 1s linear infinite', transformOrigin: 'center' }}>
                  <path d="M21 12a9 9 0 1 1-6.219-8.56"/></svg>VERIFICANDO...</>
              : 'INICIAR SESIÓN'
            }
          </button>
