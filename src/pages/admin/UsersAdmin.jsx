    ...
    <Overlay>
      <div style={{ width: '100%', maxWidth: 360, background: 'var(--bg-card)', border: '1px solid var(--border-mid)', borderRadius: 14, padding: 24 }}>
        ...
        <input type="password" value={pwd} onChange={e => setPwd(e.target.value)} placeholder="Nueva contraseña (mín. 8 chars)"
          style={{ width: '100%', padding: '10px 12px', background: 'var(--bg-input)', border: '1px solid var(--border)', borderRadius: 8, color: 'var(--text-primary)', fontFamily: 'DM Sans', fontSize: 13, outline: 'none', marginBottom: 14, boxSizing: 'border-box' }}
          onFocus={e => e.target.style.borderColor = 'var(--accent-border)'}
          onBlur={e  => e.target.style.borderColor = 'var(--border)'} />
        <div style={{ display: 'flex', gap: 8 }}>
          <button onClick={onClose} style={{ flex: 1, padding: '10px', background: 'var(--bg-surface)', border: '1px solid var(--border)', borderRadius: 8, cursor: 'pointer', fontFamily: 'JetBrains Mono', fontSize: 9, color: 'var(--text-muted)' }}>CANCELAR</button>
          <button onClick={handle} disabled={loading} style={{ flex: 2, padding: '10px', background: 'var(--accent)', border: 'none', borderRadius: 8, cursor: 'pointer', fontFamily: 'JetBrains Mono', fontSize: 9, color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
...
function BulkModal({ count, action, onConfirm, onCancel, loading }) {
  ...
  return (
    <Overlay>
      <div style={{ width: '100%', maxWidth: 360, background: 'var(--bg-card)', border: '1px solid var(--border-mid)', borderRadius: 14, padding: 28, textAlign: 'center' }}>
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          <button onClick={onCancel} disabled={loading} style={{ flex: 1, padding: '12px', borderRadius: 9, cursor: 'pointer', background: 'var(--bg-surface)', border: '1px solid var(--border)', fontFamily: 'JetBrains Mono', fontSize: 10, color: 'var(--text-muted)' }}>CANCELAR</button>
          <button onClick={onConfirm} disabled={loading} style={{ flex: 2, padding: '12px', borderRadius: 9, cursor: loading ? 'wait' : 'pointer', background: loading ? 'var(--bg-surface)' : cfg.bg, border: `1px solid ${loading ? 'var(--border)' : cfg.border}`, fontFamily: 'JetBrains Mono', fontSize: 10, color: loading ? 'var(--text-muted)' : cfg.color, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
  ...
  const handleCreate = async () => {
    ...
    try {
      await api.post('/admin/users/create', {
        ...form,
        area: form.rol === 'supervisor' ? form.area : null,
      });
      onCreated();
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <h3 style={{ fontFamily: 'Bebas Neue', fontSize: 22, color: 'var(--text-primary)', letterSpacing: '0.1em' }}>
              CREAR MIEMBRO
            </h3>
        ...
        <p style={{ fontFamily: 'JetBrains Mono', fontSize: 9, color: 'var(--text-muted)', marginBottom: 20 }}>
          Los usuarios normales se registran solos — aquí solo técnicos y supervisores
        </p>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              {[
                ...
                return (
                  <button key={r.key} onClick={() => set('rol', r.key)} style={{
                    ...
                    background: active ? rrc.bg : 'var(--bg-surface)',
                    border: `1px solid ${active ? rrc.border : 'var(--border)'}`,
                  }}>
                    <span style={{ color: active ? rrc.color : 'var(--text-muted)' }}>{r.icon}</span>
                    <span style={{ fontFamily: 'JetBrains Mono', fontSize: 10, color: active ? rrc.color : 'var(--text-secondary)', display: 'block', letterSpacing: '0.06em', fontWeight: active ? 600 : 400 }}>
                      {r.label.toUpperCase()}
                    </span>
                    <span style={{ fontFamily: 'JetBrains Mono', fontSize: 8, color: 'var(--text-faint)', display: 'block' }}>{r.desc}</span>
                  </button>
                );
              })}
            </div>
          ...
          <div>
            ...
            <input value={form.nombre_completo} onChange={e => set('nombre_completo', e.target.value)} placeholder="Nombre completo"
              style={inputStyle} onFocus={e => e.target.style.borderColor = 'var(--accent-border)'} onBlur={e => e.target.style.borderColor = 'var(--border)'} />
          </div>

          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            <div>
              <p style={{ fontFamily: 'JetBrains Mono', fontSize: 8, color: 'var(--text-muted)', marginBottom: 6, letterSpacing: '0.1em' }}>CORREO</p>
              <input type="email" value={form.correo_electronico} onChange={e => set('correo_electronico', e.target.value)} placeholder="correo@ejemplo.com"
                style={inputStyle} onFocus={e => e.target.style.borderColor = 'var(--accent-border)'} onBlur={e => e.target.style.borderColor = 'var(--border)'} />
            </div>
            <div>
          ...
          {form.rol === 'supervisor' && (
            <div style={{ padding: '12px 14px', background: 'var(--amber-soft)', border: '1px solid var(--amber-border)', borderRadius: 10 }}>
              ...
              <input value={form.area} onChange={e => set('area', e.target.value)} placeholder="Ej: Piura Norte, Zona Centro, Sector Sur..."
        ...
        <div style={{ display: 'flex', gap: 10, marginTop: 20 }}>
          <button onClick={onClose} disabled={loading} style={{ flex: 1, padding: '12px', background: 'var(--bg-surface)', border: '1px solid var(--border)', borderRadius: 9, cursor: 'pointer', fontFamily: 'JetBrains Mono', fontSize: 10, color: 'var(--text-muted)' }}>
            CANCELAR
          </button>
          <button onClick={handleCreate} disabled={loading} style={{
            flex: 2, padding: '12px',
            background: loading ? 'var(--bg-surface)' : rc.bg,
            border: `1px solid ${loading ? 'var(--border)' : rc.border}`,
            borderRadius: 9, cursor: loading ? 'wait' : 'pointer',
            fontFamily: 'JetBrains Mono', fontSize: 10,
            color: loading ? 'var(--text-muted)' : rc.color,
            fontWeight: 600, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, transition: 'all .2s',
          }}>
...
function AreaModal({ user, onClose, onDone }) {
  ...
  const guardar = async () => {
    setLoading(true);
    try {
      await adminService.updateArea(user.id_usuario, area.trim() || null);
      onDone('Área actualizada');
    } catch (e) { setErr(e.error || 'Error al actualizar'); setLoading(false); }
  };

  return (
    <Overlay>
      <div style={{ width: '100%', maxWidth: 360, background: 'var(--bg-card)', border: '1px solid var(--border-mid)', borderRadius: 14, padding: 24 }}>
        <h3 style={{ fontFamily: 'Bebas Neue', fontSize: 22, color: 'var(--text-primary)', marginBottom: 4, letterSpacing: '0.1em' }}>EDITAR ÁREA</h3>
        <p style={{ fontFamily: 'JetBrains Mono', fontSize: 10, color: 'var(--text-muted)', marginBottom: 16 }}>{user.nombre_completo}</p>
        {err && (
        ...
        <p style={{ fontFamily: 'JetBrains Mono', fontSize: 8, color: 'var(--text-muted)', marginBottom: 6, letterSpacing: '0.1em' }}>ÁREA ASIGNADA</p>
        <input value={area} onChange={e => setArea(e.target.value)}
          placeholder="Ej: Zona Norte, Sector Centro..."
          style={{ width: '100%', padding: '10px 12px', background: 'var(--bg-input)', border: '1px solid var(--border)', borderRadius: 8, color: 'var(--text-primary)', fontFamily: 'DM Sans', fontSize: 13, outline: 'none', marginBottom: 6, boxSizing: 'border-box' }}
        ...
        <p style={{ fontFamily: 'JetBrains Mono', fontSize: 8, color: 'var(--text-faint)', marginBottom: 14 }}>
          Deja vacío para quitar el área asignada
        </p>
        <div style={{ display: 'flex', gap: 8 }}>
          <button onClick={onClose} style={{ flex: 1, padding: '10px', background: 'var(--bg-surface)', border: '1px solid var(--border)', borderRadius: 8, cursor: 'pointer', fontFamily: 'JetBrains Mono', fontSize: 9, color: 'var(--text-muted)' }}>CANCELAR</button>
          <button onClick={guardar} disabled={loading} style={{ flex: 2, padding: '10px', background: 'var(--amber)', border: 'none', borderRadius: 8, cursor: loading ? 'wait' : 'pointer', fontFamily: 'JetBrains Mono', fontSize: 9, color: '#000', fontWeight: 600, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
            {loading ? <><Spinner/>GUARDANDO...</> : 'GUARDAR ÁREA'}
          </button>
        </div>
      </div>
    </Overlay>
...
export default function UsersAdmin() {
  ...
  const load = useCallback(async (params = {}) => {
    ...
    try {
      ...
      setUsers(filtered); setTotal(tot);
    } catch (e) { console.error(e); setUsers([]); setTotal(0); }
    setLoading(false);
  }, []);

  useEffect(() => {
    const timer = setTimeout(() => { load({ search: searchInput, filter, dateFrom, dateTo }); }, 400);
  ...
  const allSelected = safeUsers.filter(u => u.rol !== 'admin').length > 0 &&
    safeUsers.filter(u => u.rol !== 'admin').every(u => selected.has(u.id_usuario));
  ...
  const handleBulkConfirm = async () => {
    setBulkLoading(true);
    try { const r = await adminService.bulkAction([...selected], bulkModal); showMsg(r.message, bulkModal === 'delete' ? 'warning' : 'success'); clearSelection(); load({ search: searchInput, filter, dateFrom, dateTo }); }
    ...
    <div style={{ padding: '24px 28px' }} className="anim-fade">
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12, marginBottom: 20 }}>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
          {msg && (
            <span style={{ fontFamily: 'JetBrains Mono', fontSize: 9, padding: '6px 12px', borderRadius: 8, color: msgType === 'success' ? 'var(--green)' : 'var(--amber)', background: msgType === 'success' ? 'var(--green-soft)' : 'var(--amber-soft)', border: `1px solid ${msgType === 'success' ? 'var(--green-border)' : 'var(--amber-border)'}` }}>
              {msg}
            </span>
          )}
      ...
      <Card style={{ marginBottom: 14 }}>
        ...
        <div style={{ display: 'flex', gap: 7, marginTop: 12, flexWrap: 'wrap', alignItems: 'center' }}>
          {[['all','TODOS'],['pending','PENDIENTES'],['active','ACTIVOS'],['inactive','INACTIVOS'],['admin','ADMINS']].map(([key, label]) => (
            <button key={key} onClick={() => setFilter(key)} style={{ padding: '5px 12px', borderRadius: 7, cursor: 'pointer', background: filter === key ? 'var(--accent-soft)' : 'var(--bg-surface)', border: `1px solid ${filter === key ? 'var(--accent-border)' : 'var(--border)'}`, fontFamily: 'JetBrains Mono', fontSize: 8, letterSpacing: '0.08em', color: filter === key ? 'var(--accent)' : 'var(--text-muted)', transition: 'all .18s' }}>
              {label}
            </button>
          ))}
                    ...
                    <tr key={u.id_usuario}
                      ...
                      <td style={{ padding: '10px 10px' }}>
                        {u.rol === 'admin' ? (
                          ...
                          <button onClick={() => setResetUser(u)} style={{ padding: '4px 9px', borderRadius: 5, cursor: 'pointer', background: 'var(--bg-surface)', border: '1px solid var(--border)', fontFamily: 'JetBrains Mono', fontSize: 7, color: 'var(--text-muted)' }}>PWD</button>
                        ) : (
                          <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap' }}>
                            ...
                            {u.rol === 'supervisor' && (
                              <button onClick={() => setAreaUser(u)} style={{ padding: '4px 9px', borderRadius: 5, cursor: 'pointer', background: 'var(--amber-soft)', border: '1px solid var(--amber-border)', fontFamily: 'JetBrains Mono', fontSize: 7, color: 'var(--amber)' }}>ÁREA</button>
                            )}
                            <button onClick={() => setResetUser(u)} style={{ padding: '4px 9px', borderRadius: 5, cursor: 'pointer', background: 'var(--bg-surface)', border: '1px solid var(--border)', fontFamily: 'JetBrains Mono', fontSize: 7, color: 'var(--text-muted)' }}>PWD</button>
                            <button onClick={() => remove(u)} style={{ padding: '4px 9px', borderRadius: 5, cursor: 'pointer', background: 'var(--accent-soft)', border: '1px solid var(--accent-border)', fontFamily: 'JetBrains Mono', fontSize: 7, color: 'var(--accent)' }}>✕</button>
                          </div>
                        )}
                      </td>
                    </tr>
      ...
      {showCreate && <CreateUserModal onClose={() => setShowCreate(false)} onCreated={() => { setShowCreate(false); showMsg('Miembro del equipo creado'); load({ search: searchInput, filter, dateFrom, dateTo }); }} />}
    </div>
  );
}